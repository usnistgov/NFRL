...
### OpenCV for Windows
Required packages may be found here: `opencv.org/releases/`.  Click on the `opencv.org/releases/` Windows link to
download the self-extractor.  For Windows, it is not necessary to build the development and runtime libraries.
...
Note: to configure OpenCV to build the single, monolithic, "world" lib which inlcudes all OpenCV modules for easy
