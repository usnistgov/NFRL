...
{
  std::vector<uchar> smallerImgData, largerImgData;
...
/** Wrapper method to retrieve the padded, colorized, overlaid, registered
 *  image from memory.
 * 
 * @return encoded image as type OpenCV cv::Mat
 */
cv::Mat Registrator::getColorOverlaidRegisteredImages()
{
  std::vector<uchar> vec = _r2->getColorOverlaidRegisteredImages();
  return cv::imdecode( cv::Mat(vec), cv::IMREAD_UNCHANGED );
}

/** Wrapper method to retrieve the cropped, fixed image from memory.
 * 
 * @return encoded image as type OpenCV cv::Mat
 */
cv::Mat Registrator::getCroppedFixedImage()
{
  std::vector<uchar> vec = _r2->getCroppedFixedImage();
  return cv::imdecode( cv::Mat(vec), cv::IMREAD_UNCHANGED );
}

/** Wrapper method to retrieve the cropped, registered image (that was moved)
 *  from memory.
 * 
...
{
  std::vector<uchar> vec = _r2->getCroppedRegisteredImage();
  return cv::imdecode( cv::Mat(vec), cv::IMREAD_UNCHANGED );
}
...
{
  std::vector<uchar> vec = _r2->getPngBlob();
  return cv::imdecode( cv::Mat(vec), cv::IMREAD_UNCHANGED );
}

/** Wrapper method to retrieve the padded, grayscale Fixed image from memory.
 * 
 * @return encoded image as type OpenCV cv::Mat
 */
cv::Mat Registrator::getPaddedFixedImg()
{
  std::vector<uchar> vec = _r2->getPaddedFixedImg();
  return cv::imdecode( cv::Mat(vec), cv::IMREAD_UNCHANGED );
}

/** Wrapper method to retrieve the padded, registered, grayscale Moving image
 *  from memory.
 * 
 * @return encoded image as type OpenCV cv::Mat
 */
...
{
  std::vector<uchar> vec = _r2->getPaddedRegisteredMovingImg();
  return cv::imdecode( cv::Mat(vec), cv::IMREAD_UNCHANGED );
}
